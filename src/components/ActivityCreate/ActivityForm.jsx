...
const metrics = [
  { label: '钻石数', field: 'stage1Diamond', value: '1000' },
  { label: '直播时长 (小时)', field: 'stage1Hours', value: '20' },
  { label: '有效开播天数', field: 'stage1Days', value: '8' },
];
const checkboxes = ['钻石数', 'PK 钻石数', '直播时长', '有效开播天数', 'PK 场次', 'PK 胜利场次'];

export default function ActivityForm({ formData, setFormData, step, onNext, onPrev }) {
  ...
  return (
    <div className="space-y-6">
        ...
        <section>
          ...
          <div className="flex items-center border-b border-gray-200 mb-4">
            {Array.from({ length: formData.stageCount || 1 }, (_, i) => i + 1).map(n => {
              const isActive = (formData.activeStageTab || 'stage1') === `stage${n}`;
              return (
                <div key={n} className="relative w-[120px] pb-2.5 pt-1 flex items-center justify-center cursor-pointer" onClick={() => updateField('activeStageTab', `stage${n}`)}>
                  <span className={`text-sm font-medium transition-colors ${isActive ? 'text-slate-800' : 'text-slate-400 hover:text-slate-600'}`}>赛段 {n}</span>
                  {(formData.stageCount || 1) > 1 && (
                    <button onClick={(e) => { e.stopPropagation(); updateField('stageCount', Math.max(1, (formData.stageCount || 2) - 1)); if (isActive) updateField('activeStageTab', 'stage1'); }} className="ml-3 text-slate-400 hover:text-slate-600 transition-colors"><X className="w-3.5 h-3.5" /></button>
                  )}
                  {isActive && <motion.div layoutId="stageTabV2" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#34c2c1] rounded-full" transition={{ type: 'tween', ease: 'easeInOut', duration: 0.25 }} />}
                </div>
              );
            })}
          ...
          {/* Stage Content */}
          {(formData.activeStageTab || 'stage1') === 'stage1' && <StageOne formData={formData} updateField={updateField} />}
          {(formData.activeStageTab || 'stage1') === 'stage2' && <StageTwo formData={formData} updateField={updateField} stage2Metrics={stage2Metrics} toggleMetric={toggleMetric} />}
          {(formData.activeStageTab || 'stage1') === 'stage3' && <div className="space-y-3"><h4 className="text-sm font-semibold text-slate-800">赛段 3</h4><p className="text-xs text-slate-400">赛段 3 配置项即将上线</p></div>}
        </section>
      )}
      ...
      {/* Navigation Buttons */}
      <div className="flex justify-end items-center gap-x-4 mt-6 pt-4 border-t border-gray-100 select-none">
        {step > 1 && (
          <button onClick={onPrev} className="text-sm font-semibold text-slate-500 hover:text-slate-800 transition-colors">上一步</button>
        )}
        <button onClick={onNext} className="h-8 px-5 bg-[#34c2c1] hover:bg-[#2da9a8] active:scale-95 rounded-md text-white text-sm font-semibold transition-all flex items-center justify-center">
          {step === 3 ? '活动预览' : '下一步'}
        </button>
      </div>
    </div>
...
function StageOne({ formData, updateField }) {
  return (
    <div className="space-y-3">
      <div>
        <label className="text-xs text-slate-500 mb-1 block">赛段玩法</label>
        ...
        <p className="text-xs text-slate-400 mt-1">创作者完成全部设置任务后可晋级或获奖，提升参与度</p>
      </div>
      <div>
        <label className="text-xs text-slate-500 mb-1.5 block">任务类型 <span className="text-rose-400">*</span></label>
        <RadioGroup options={[{ value: 'transition', label: '主播跃迁任务' }, { value: 'active', label: '主播活跃任务' }, { value: 'custom', label: '自定义任务' }]} value={formData.stage1TaskType || 'custom'} onChange={v => updateField('stage1TaskType', v)} />
      </div>
      <div>
        ...
        <div className="border border-gray-200 rounded-lg overflow-hidden">
          <table className="w-full"><thead><tr className="bg-gray-50/50 border-b border-gray-200"><th className="text-left text-xs text-slate-500 font-medium px-3 py-2">任务指标</th><th className="text-left text-xs text-slate-500 font-medium px-3 py-2">阶段 1 目标</th><th className="text-right text-xs font-medium px-3 py-2"><button className="text-[#34c2c1] hover:text-[#2aa8a7]">+ 添加阶段目标</button></th></tr></thead>
            <tbody>
              {[{ label: '钻石数', field: 'stage1Diamond', value: '1000' },{ label: '直播时长 (小时)', field: 'stage1Hours', value: '20' },{ label: '有效开播天数', field: 'stage1Days', value: '8' }].map(item => (
                <tr key={item.field} className="border-b border-gray-100 last:border-0">
                  <td className="px-3 py-2"><div className="relative"><select className="appearance-none w-full bg-[#F2F2F2] border border-gray-200 rounded-lg h-8 pl-2.5 pr-6 text-sm text-slate-700 focus:outline-none cursor-pointer"><option>{item.label}</option></select><ChevronDown className="absolute right-1.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none" /></div></td>
        ...
        <button className="flex items-center gap-1 text-xs text-[#34c2c1] hover:text-[#2aa8a7] font-medium mt-2"><Plus className="w-3 h-3" />添加指标</button>
      </div>
      <div>
        <label className="text-xs text-slate-500 mb-1 block">比赛时间 <span className="text-rose-400">*</span></label>
        <div className="flex items-center gap-2">
          <div className="relative"><select className="appearance-none bg-[#F2F2F2] border border-gray-200 rounded-lg h-9 pl-3 pr-7 text-sm text-slate-700 focus:outline-none cursor-pointer"><option>(UTC +09:00) Asia/Khandyga</option></select><ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" /></div>
          <div className="flex items-center gap-2">
            ...
            <div className="relative"><Calendar className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none" /><input type="date" value={formData.endDate} onChange={e => updateField('endDate', e.target.value)} className="bg-[#F2F2F2] border border-gray-200 rounded-lg h-9 pl-8 pr-3 text-sm text-slate-700 focus:outline-none" /></div>
          </div>
        </div>
      </div>
    </div>
...
function StageTwo({ formData, updateField, stage2Metrics, toggleMetric }) {
  return (
    <div className="space-y-3">
      <div>
        <label className="text-xs text-slate-500 mb-1 block">赛段玩法</label>
        ...
        <p className="text-xs text-slate-400 mt-1">按所选指标累计排名，激发竞争，提升核心指标</p>
      </div>
      <div>
        <label className="text-xs text-slate-500 mb-2 block">积分指标 <span className="text-rose-400">*</span></label>
        <div className="flex flex-wrap gap-x-5 gap-y-2">
          {['钻石数', 'PK 钻石数', '直播时长', '有效开播天数', 'PK 场次', 'PK 胜利场次'].map(opt => (
        ...
        {stage2Metrics.includes('钻石数') && (
          <div className="flex items-center gap-2 text-sm text-slate-500 mt-3"><span className="text-slate-600">钻石数</span><span className="text-slate-400">1 钻石 =</span><input type="number" className="w-24 bg-[#F2F2F2] border border-gray-200 rounded-lg h-8 px-2.5 text-sm text-slate-700 focus:outline-none" defaultValue="100" /><span className="text-slate-400">积分</span></div>
        )}
      </div>
      <div>
        <label className="text-xs text-slate-500 mb-1 block">比赛时间 <span className="text-rose-400">*</span></label>
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-400 text-sm">~</span>
          <div className="relative"><Calendar className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none" /><input type="date" value={formData.stage2End || ''} onChange={e => updateField('stage2End', e.target.value)} className="bg-[#F2F2F2] border border-gray-200 rounded-lg h-9 pl-8 pr-3 text-sm text-slate-700 focus:outline-none" /></div>
        </div>
      </div>
    </div>
  );
